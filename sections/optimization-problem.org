...
This is effectively the cost of selecting queue $q_i$. In [[#sec:sa-example]], a particular choice for the assignment cost
...
form of [[eq:penalty]] with and added scalar, $z_p$, is employed so that the cost of deviating from the threshold heavily
