...
$\iota_h^{v_i} \in \{0,1\} = 1$ when $\mu_h^{v_i}$ and $\theta_h^{v_i}$ are both active and is covered in [[#sec:objective-function]].
...
in [[#sec:constraints]]. Thus, when $\phi_i$ is active, the value of second term is $C(\eta_i - m \kappa_i)^2$ and $0$ when inactive.
Note the variables $\phi_i$ and $\eta_i$ being multiplied together in [[eq:ac]]. These are decision variables and when multiplied
