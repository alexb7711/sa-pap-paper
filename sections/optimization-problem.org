...
function is broken down into four major components: consumption cost, _demand cost_, assignment cost, as well as a
...
[[#sec:objective-function]]. The constraints will then be introduced in [[#sec:constraints]].
...
are the values that the SA algorithm has the freedom to manipulate and are discussed in [[#sec:decision-variables]].
...
discussed in [[#sec:decision-variables]]. $dt_h$ is the discrete time step used for calculating the demand cost. $\epsilon_q$ is
...
further discussed in [[#sec:simulated-annealing]]. Lastly, $r_q$ represents the power supplied from the charger in queue
$q$.
...
[[#sec:objective-function]]. $\phi_i$ is a boolean decision variable, $\phi_i \in \{0,1\}$, that either enables or disables the
charge penalty defined the objective function. The charge penalty method is discussed in [[#sec:objective-function]].
$\sigma_{ij}$ and $\psi_{ij}$ are used to indicate whether a visit pair $(i, j)$ overlap the same space as show in
[[fig:spacial-and-temporal-constr]]. More formally, [[eq:bus-spat-temp]] describes the relationship that $\sigma_{ij}$ and $\psi_{ij}$
