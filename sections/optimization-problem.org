...
This cost is effectively the cost on the choice the choice of selecting queue $q_i$. In the [[#sec:sa-example]], a
...
penalty proportional to the difference of the SOC and the threshold squared. Using the form of [[eq:penalty]] with and added
...
[[eq:consumption-cost]], $z_c$ may also be used simply as a weight for the consumption cost. Within the summation, the
