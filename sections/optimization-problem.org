...
penalty function. The mechanisms that enables or disables $\phi_i$ is derived in [[#sec:constraints]].
...
[[eq:pdem]] then begins with a value of $p_{fix}$ from which only increases if $p_{15}(t) > p_{fix}$. To write the total
...
To write the usage cost, [[eq:consumption-cost]] and [[eq:pd-dis]] are added together to create [[eq:pc]].
...
bus $b_i$. \ref{seq:c7} ensures that the bus is not being over-charged. \ref{seq:8} ensures that the BEB's SOC does not
go below 0. \ref{seq:c9} and \ref{seq:c10} are used to enable and disable the penalty method in [[eq:ac]]. This is done by
...
constraints define three $n_T \tims n_Q$ matrices that enforce $\iota_h^q = 1$ if $q = v_i$ and $u_i \le dt_h \le d_i$, $\iota_h^q =
0$ otherwise. \ref{seq:c12} and \ref{seq:c13} utilizes big-M notation to ensure that the initial charge time for visit
$i$, $u_i$, is greater than or equal to the discrete time, $0.25 \cdot h$. That is, $\mu_h^{v_i} = 1$ if $u_i dt_h$,
$u_h^{v_i} = 0$ otherwise. Similarly, \ref{seq:c14} and \ref{seq:c15} ensures that the final charge time, $d_i$, is less
than or equal to the next discrete time step, $dt_h$. The final constraint \ref{seq:c16} is a $n_T \times n_Q$ matrix that
represents whether charger $v_i \in \Qset$ is active. Note that $\circ$ represents the Hadamard Product
[cite:@horn-1985-matrix-analy]. Let $A$ and $B$ be two $n_M \times n_N$ matrices. The Hadamard Product is defined as the
element-wise product of the two matrices, $A \circ B = [a_{ij}b_{ij}]$.
