...
[[#sec:constraints]]. Thus, when $\phi_i$ is active, the value of second term is $C(\eta_i - m \kappa_i)^2$ and $0$ when inactive. Note
the variables $\phi_i$ and $\eta_i$ being multiplied together in [[eq:ac]]. These are decision variables and when multiplied
