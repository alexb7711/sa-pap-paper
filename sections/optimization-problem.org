...
$\psi_{ij}$ is active ($\psi_{ij} = 1$). These variables will be further elaborated on in [[#sec:constraints]].
...
The cost of the chosen charge times is implemented by utilizing a penalty method. Using the form of [[eq:penalty]] with and
