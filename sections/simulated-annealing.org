...
with $\Tau[0] = \Tau_0$ and $\Delta_0 = 1/2\; C^\circ$ in [[fig:cool]]. The value of $\Delta_0$ can vary between $[0,1)$. A geometric
cooling schedules is as defined in [[eq:cool1]]. The cooling schedule type most widely used in practice
...
where $\beta = 0.01$ in [[fig:cool]]. The value of $\beta$ may vary anywhere between the rang of $[0, 1)$.
...
\caption{Charge schedule generation algorithm} \label{alg:charge-schedule-generation}
    ...
    \Begin
    {
        ...
        \ForEach (\tcc*[f]{Select an unscheduled BEB visit from a randomly indexed set of visits}) {$\I_i \in \{\ \U_{\I} \}$}
        {
...
distribution. Let $G$ denote the number of primitive generators available and let =PGF= be the primitive generation
...
#  LocalWords:  metaheuristic linearities BEBs
