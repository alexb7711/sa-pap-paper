...
([[#sec:sa-objective-function]]), $t_m$ is current temperature, $\I$ is the current solution, and $\bar{\I}$ is the new
...
case of the problem statement made in [[#sec:sa-problem-description]], five primitive generation mechanism are used: new visit,
