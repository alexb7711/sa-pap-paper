...
:END:
In SA, the algorithm stores a candidate solution. Let the stored solution be denoted as the active solution. Each
iteration a new candidate solution is generated compared to the active solution to determine if the new solution should
replace the active solution. In order to determine if the active solution is to be replaced, an acceptance criteria must
be defined. In SA, a new candidate solution that is more fit than active solution is always accepted as the new active
...
$\exp(-\frac{J(x) - J(x')}{\Tau})$ where $J(\cdot)$ is the objective functions described in [[#sec:objective-function]] and
$\Tau$ is current temperature. The probability of acceptance is a function of the cooling equation just described and
...
The function =findFreeTime= in \ref{alg:new-visit} is defined in \ref{alg:find-free-time}. $L$ and $U$ are the lower
and upper bound of the time between scheduled times. The possible use cases are depicted in [[fig:find-free]]. In each case
depicted by [[fig:find-free]], the red line shows the arrival and departure time for an arbitrary bus visit, $i$. The blue
