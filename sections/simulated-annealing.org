...
with $t_0 = \Tau_0$ and $\beta_0 = 1/2\; C^\circ$ in [[fig:cool]]. The value of $\beta_0$ may vary anywhere in the range $\beta_0 \in \mathbb{R}^+$. A
...
where $\beta = 0.995$ in [[fig:cool]]. The gain variable, $\beta$, may vary anywhere between the range $[0,1)$. An exponential
