...
The function =findFreeTime= is the algorithm that determines whether a visit's time at the station $[a_i, e_i]$ can be
placed in the time availability, $C$, of charger $q$. The algorithm is defined in \ref{alg:find-free-time}. Let $L$ and
$U$ be the lower and upper bound of the time between scheduled times. The set of cases is shown in [[fig:find-free]]. In
each case depicted by [[fig:find-free]], the red lines depict the arrival and departure time for a BEB visit, $i$. The blue
