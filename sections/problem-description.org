...
bus at the station. Let $\mathcal{Z}$ be the set of integers, given a set of bus arrivals to a charging station $i \in \{1,..., I\} = \Iset \subset \mathcal{Z}$ with a set of
...
while at the station is required to supply enough charge for each route with
...
time horizon $T$ as well as minimize the demand cost and consumption cost. The objective
