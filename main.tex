...
detach time for visit \(i\), \(\phi_i(x)\) is a piecewise function utilized with the penalty method, \(\nu\) is a vector of the
minimum charge percentage allowed for each BEB \(\kappa_i\) is the battery capacity for visit \(i\), and \(\eta_i\) is the initial
charge for visit \(i\).

\begin{equation}
\label{eq:ac}
\AC = \sum_{i=1}^{n_I} \Big(\epsilon_{v_i}r_{v_i} + \frac{1}{2} C \phi_i(\eta_i - m \kappa_i) \Big)
\end{equation}
  ...
  \KwOut{$\bar{\Sol}$}
    ...
    \Begin
    {
      ...
      $C \leftarrow \bar{C}_{i.v_i}$\tcc*{Get the time availability of the visit just purged}
        ...
        $\bar{\C} \leftarrow \C$\tcc*{Reassign the original visit if the generator fails}
        \Return{($\varnothing$)}\tcc*{Return nothing}
    }
  \end{algorithm}
