...
when the initial charge, \(\eta_i\), is less than the allowed minimum charge, \(m\kappa_{b_i}\). This is further discussed in
\ref{sec:constraints}. Thus, when \(\phi_i\) is active, the value of second term is \(C(\eta_i - m \kappa_i)^2\) and \(0\) when inactive. Note
the variables \(\phi_i\) and \(\eta_i\) being multiplied together in \ref{eq:ac}. These are decision variables and when multiplied
