...
\label{sec:results}
The schedules generated by their respective framework is presented in \ref{fig:schedule}.

The most obvious comparison
between the different schedules is the minimization of the total chargers utilized by the schedule and the perceived
organization of the Qin, MILP PAP, and heuristic SA strategy as compared to the quick SA strategy. The Qin-Modified
...
framework generated a schedule that utilizes three fast charges and four slow chargers as shown in
\ref{subfig:schedule-milp}. This schedule utilizes at most two fast charges and four slow chargers simultaneously as
represented in \ref{subfig:slow-charger-usage-milp-qinn}. The heuristic SA strategy created its schedule with eight slow
charger queues and four fast charging queues as shown in \ref{subfig:schedule-heuristic-sa}. The heuristic approach uses
a peak of seven slow chargers and two fast chargers used at any given moment as shown in
\ref{subfig:fast-charger-usage-sa} and \ref{subfig:slow-charger-usage-sa}. The quick strategy for the SA algorithm
created a schedule utilizing fifteen slow and fast chargers as is demonstrated in \ref{subfig:schedule-quick-sa}. The
schedule only utilizes five slow chargers and four fast chargers simultaneously as shown in
\ref{subfig:fast-charger-usage-sa} and \ref{subfig:slow-charger-usage-sa}. That is to say, the Qin-Modified, MILP PAP,
and the heuristic SA schedule were more efficient in the minimization of the charger count than the quick SA technique.
The MILP produced a schedule with a three charger gap in the fast queues, where the intermediate queues were never used.
The heuristic SA, while being possibly being able to move some assignments to a lower charge queue index, did contain
any gaps of unused queues. The Qin-Modified utilized the fewest chargers overall, but also suffers a lack of optimality
in its packing of the schedule.

\begin{table}[htbp]
\caption{\label{tab:charge-count}Table of mean, min, and max chargers utilized throughout the time horizon.}
\centering
\begin{tabular}{|l|l|l|l|l|l|l|l|l|}
\hline
 & \multicolumn{2}{l|}{MILP} & \multicolumn{2}{l|}{QM} & \multicolumn{2}{l|}{Exhaustive} & \multicolumn{2}{l|}{Quick} \\
\hline
...
\ref{fig:charge} depicts the initial SOC for each visit throughout the simulation of each framework. The MILP PAP
requires each BEB to stay above an SOC of 25\% while the quick and heuristic SA approaches heavily penalize a schedule
...
50 kWh at the end of the working day (\ref{subfig:sa-quick-charge}) and the heuristic approach reach the 50 kWh SOC mark
at about the sixteenth, eighteenth, and at the end of the working day (\ref{subfig:sa-heuristic-charge}). The Qin model
allowed the SOC of three BEBs to reach an SOC of 0\% as shown in \ref{subfig:qin-charge}. As to be expected, no model
allowed the SOC to go above the battery capacity, \(\kappa\).

\begin{center}
\begin{tabular}{|l|l|l|l|l|}
\hline
 & Exhaustive & Quick & MILP & QM \\
\hline
Mean & 189.525113828846 & 229.449476346154 & 179.580229533742 & 253.049696882376 \\
Min & 6.3432083 & 29.862568 & 96.9999999999463 & 0 \\
Max & 388 & 388 & 388 & 368.6 \\
\hline
\end{tabular}
\end{center}
