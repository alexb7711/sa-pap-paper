...
The \texttt{findFreeTime} algorithm behaves as follows. Lines 1 and 2 of \ref{alg:find-free-tim} extract the lower and upper
...
\caption{Slide Visit Algorithm} \label{alg:slide-visit}
    ...
    \Begin
    {
      ...
      $C \leftarrow \bar{C}_{i.v_i}$\tcc*{Get the time availability of the visit just purged}
        ...
        $\I_i \leftarrow$ ($\I_{i.q},u,d,\Sol_{\C}$)\tcc*{Reassign the original visit if the generator fails}
        \Return{($\varnothing$)}\tcc*{Return nothing}
    }
  \end{algorithm}
