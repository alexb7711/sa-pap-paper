...
concept of ``arrivals'' is derived from the PAP (, ). This idea of arrivals is useful in the
...
disabled by \(\phi_i\) (, a). \(\phi_i\) is enabled when the initial charge, \(\eta_i\), is less
...
traditional MILP solver, this would require linearization (, a); however, because SA handles
nonlinearities easily these bilinear terms will be ignored (, a).
...
overcome nonlinearities (, a, , a). This model is named after its
analogized process where a crystalline solid is heated then allowed to cool very slowly until it achieves its most
regular possible crystal lattice configuration (, a). There are five key components to SA:
...
(, a, , a)]. There are three basic types of cooling equations as
...
disjoint (, a). The inverse of a disjoint set can be found by the De Morgan Law as shown
...
in \ref{alg:schedule-generation}. Recall from \ref{sec:input-variables}, \(\Jset_B \subset \I\) is used to indicate the set of visit
indices for bus \(b\).
...
(, a). The method that will be employed to exploit the given solution is as follows:
...
related to any of the variables presented for SA PAP thus far. Consider \ref{alg:sa-pseudo} (, a).
