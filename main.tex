...
calculating the demand cost. \hl{That is, $\iota_h^{v_i} \in \{0,1\} = 1$ when $\mu_h^{v_i} = 1$ and $\theta_h^{v_i} = 1$.} This topic will be covered in more depth in \ref{sec:objective-function}.
...
used to scale the penalty function. The mechanism that enables or disables \(\phi_i\) is derived in \ref{sec:constraints}.
