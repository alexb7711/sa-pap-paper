...
On the quantitative side of the minimization/packing discussion, the Qin-Modified schedule utilizes four fast and slow
chargers as can be seen in \ref{subfig:schedule-qin}. The MILP PAP framework generated a schedule that utilizes three
fast charges and four slow chargers as shown in \ref{subfig:schedule-milp}. The heuristic SA strategy created its
...
in \ref{subfig:schedule-quick-sa}. That is to say, the Qin-Modified, MILP PAP, and the heuristic SA schedule were able
to more effectively minimize the charger count than the quick SA technique. The MILP produced a schedule with a three
charger gap in the fast queues, where the intermediate queues were never used. The heuristic SA, while possibly being
able to move some assignments to a lower charge queue index, did not contain any gaps of unused queues. The Qin-Modified
utilized the fewest chargers overall, but also suffers a lack of optimality in its packing of the schedule. Thus, none
of the methods were able to fully minimize the packing constraint. Note that the SA algorithm has no guarantee of
optimality; therefore, post-processing could be applied to further minimize the charger indices.

A table of the mean, minimum, and maximum instantaneous charger use is shown in \ref{tab:charge-count}. The first row
represents the mean instantaneous charger use throughout the time horizon. This is meant to be a measure of how often
the schedule utilized their chargers in parallel. The Qin-Modified had the smallest mean instantaneous slow usage at
0.701 chargers followed by the quick SA strategy at 1.494, then the MILP at 1.877, and then the exhaustive SA strategy
having the largest mean at 1.8 chargers.

\begin{table}[htbp]
\caption{\label{tab:charge-count}Table of mean, min, and max chargers instantaneous charger usage throughout the time horizon.}
\centering
\begin{tabular}{|l|l|l|l|l|l|l|l|l|}
\hline
 & \multicolumn{2}{l|}{MILP} & \multicolumn{2}{l|}{Qin-Modified} & \multicolumn{2}{l|}{Exhaustive} & \multicolumn{2}{l|}{Quick} \\
\hline
...
\ref{fig:charge} depicts the initial SOC for each visit throughout the simulation of each framework and
\ref{tab:charge}. The MILP PAP requires each BEB to stay above an SOC of 25\% while the quick and heuristic SA
approaches heavily penalize a schedule for allowing a BEB to go below the 25\% SOC threshold. The MILP PAP was able to
successfully keep the SOC above the threshold (\ref{subfig:milp-charge}) while both SA approaches were not. The SOC of
the quick SA approach dropped to the 50 kWh at the end of the working day (\ref{subfig:sa-quick-charge}) and the
heuristic approach reach the 50 kWh SOC mark at about the sixteenth, eighteenth, and at the end of the working day
