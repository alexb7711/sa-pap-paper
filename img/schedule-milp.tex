...
\pgfplotstableread[col sep= comma]{data/milp-schedule.csv}\scheduledatamilp

\begin{tikzpicture}
\pgfplotstablegetcolsof{\scheduledatamilp}
\pgfmathtruncatemacro\NumCols{\pgfplotsretval/3 - 1}
\begin{axis}[title={BPAP Schedule},
    scale only axis,
    ...
    \foreach \i in {0, ..., \NumCols}
    {
        ...
        \addplot +[red, only marks, mark=otimes, error bars/.cd, error bar style={solid}, x dir=plus, x explicit]
